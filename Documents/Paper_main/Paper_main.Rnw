...
On a concluding note, we would like to clarify the relationship between publication bias and Type M and Type S errors. While the two are related, they operate at two different levels. Publication bias refers to a publication system that favors statistically significant results over non-statistically significant findings. This phenomenon alone cannot explain the presence of exaggerated effects. Imagine if all studies in the literature were conducted with high statistical power, then statistically significant findings would probably not be so extreme. The problem of exaggerated effect sizes in the literature can be explained only by a combination of publication bias with studies that have low statistical power. As previously shown, statistical power and Type M and Type S errors are related to each other: low statistical power corresponds to higher Type M and Type S errors. In the same way as statistical power is a characteristic of a study design, so are Type M and Type S, however the two are qualitatively different in terms of the kind of reasoning they favor.
Statistical power is defined in terms of probability of rejecting the Null hypothesis and, even though this is based on an effect size of interest, the relationship “low power - high possibility of exaggeration” may not be straightforward for everyone. Instead, Type M and Type S errors directly quantify the possible exaggeration. Furthermore their consideration protects against another possible pitfall. It is quite widespread the knowledge that even the smallest effects can become statistically significant if the sample size is large enough. However, if a statistically significant result is found paired with a large effect size, the results may be considered robust. However, this is not true in all cases. Here, the missing piece of information is  statistical power. If power is considered, researchers would realize that a large effect was found in a context where there was a low probability to find it. What is the meaning? This interpretation is not explicitly stating an important consequence: the study may find effects that are overestimated, if statistically significant. On the contrary, this becomes immediately clear once Type M and Type S errors are considered retrospectively. In a similar fashion, considering Type M and Type S prospectively favors reason in terms of effect size rather than probability of rejecting the null when setting the sample size in a design analysis.
